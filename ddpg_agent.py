...
class Agent():
    ...
    def step(self, states, actions, rewards, next_states, dones, timestep, num_agents, accumulate=True):
        """Save experience in replay memory, and use random sample from buffer to learn."""
        
        # Pass in accumulate=False if we are already doing the next step from the ddpg function...
        ...
        if accumulate:
            for i in range(num_agents):
                add_to_memory(states[i], actions[i], rewards[i], next_states[i], dones[i])
        else:
            add_to_memory(states, actions, rewards, next_states, dones)

        # Learn, if enough samples are available in memory
        learn_from_memory(timestep)
